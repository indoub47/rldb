...
test('"checkStillExists" returns object', () => {
  expect.assertions(3);
  return checkStillExists(pool, "defect", 14, 8).then(data => {
...
test('"checkStillExists" object is inactive, returns object', () => {
  expect.assertions(3);
  return checkStillExists(pool, "defect", 43580, 8).then(data => {
...
test('"checkStillExists" returns item removed error', () => {
  expect.assertions(1);
  return checkStillExists(pool, "defect", 19, 8).catch(error => {
...
test('"checkStillExists" wrong region, returns item removed error', () => {
  expect.assertions(1);
  return checkStillExists(pool, "defect", 15020, 8).catch(error => {
    expect(error).toEqual(err);
  });
});
